...
- Browser: the advert journey uploads PDFs, adds Ada's cover letter, checks the recruiter sees it and that "View CV" opens the PDF, and that erasure removes both. `cv-upload.spec.ts` checks the messages for a wrong type, an oversized file, a textless PDF and an over-long cover letter, then submits successfully.
