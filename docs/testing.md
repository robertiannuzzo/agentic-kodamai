...
The 243 checks cover field validation, separation of duties, audit-trail replay (legal trails, self-approval, missing submission, wrong revision, wrong requisition, unsubmitted fields, each of the five fields changed after approval, older justification-only approvals, snapshot delimiter and Unicode boundaries, empty trail), routing through the `transitionAgent` container, hire provenance, all human decision branches, hold/rework lineage, required evidence, unique reference allocation, stale writes, wrong-stage operations, one publication per case, schema errors, request/question/skill mismatches, extraction failures, persistence failures, retries and payload conflicts, exact CV version matching, golden score breakdowns, empty inputs, normalization, keyword repetition, capped experience, and codec round trips/rejections.
...
Review fixes (2026-09-25), after a review of Slice 4: 243 runtime checks, one positive fixture and 20 negative fixtures. 42 HTTP tests: nine new ones cover truncated, oversized and abandoned uploads and 20 concurrent malformed ones, with the server still answering (in process and in a separate process); an idempotency key reused for another requisition; each approved field changed in the database blocking publication, and a changed advert blocking applications and decisions; multiline and Unicode fields through rework and restart; older justification-only records still working after an upgrade; and the raw database file free of erased names, CV text, cover letters and answers after withdrawal, erasure, retention and restart, including an upgrade from a WAL-mode file. Each new test was also run against the code before the fix and failed. Eight Playwright journeys, in Chrome and in Playwright's Chromium: the applicant panel is a modal dialog, opened and closed by keyboard with Tab kept inside it and focus returned (or moved to the Applications heading when a hire redraws the view); tabs move with the arrow, Home and End keys; and a new journey shows that edits to any draft field block submission until saved, that a failed save keeps them, and that the approver sees the saved values. A copy of an existing local database (schema 8, WAL) was also upgraded and used: its applicants listed, and a new PDF application to its advert was accepted.

Slice 4 (2026-09-25): the core suite is unchanged at 239 runtime checks, one positive fixture and 20 negative fixtures, since the kernel did not change. 33 HTTP tests: the 27 existing ones now submit applications as PDF uploads generated by `tests/fixtures/pdf.ts`, and six new ones in `slice4.test.ts` cover extraction with an unchanged score, private retrieval by role and tenant, every refused upload (missing, two files, renamed non-PDF, over 5 MB, corrupt, textless) with no document stored, kernel refusals and replays storing no extra document, cover-letter limits with no effect on score, erasure of the PDF and cover letter by withdrawal, erasure and retention, and the new triggers. Seven Playwright journeys: the advert journey uploads PDFs, adds a cover letter and opens the CV, and `cv-upload.spec.ts` checks the messages for each unusable file and an over-long cover letter. The finished candidate and recruiter screens were also checked in Chrome at 1280 and 375 pixels wide, including the PDF opening in a new tab.

Hardening 3.2 (2026-09-24), after a second review: 27 HTTP tests add replays of cached review and hire responses after erasure (no email or note), concurrent identical applications under an exhausted rate limit (both 201, one result), and an upgrade from a migration-6 erasure that migration 8 repairs. A fifth browser journey submits an application as one candidate, switches identity mid-flight, and fails if the result marks the next candidate as applied.
